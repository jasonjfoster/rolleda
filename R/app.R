  ...
  server <- function(input, output) {
    ...
    shiny::observe({
      ...
      roll_cov_fn <- function(fn) {
        ...
        result_dt[, get("variable") := paste(get("V2"), get("V1"), sep = "_")]
        result_dt <- data.table::dcast(result_dt, get("V3") ~ variable, fun.aggregate = mean)
        result_dt[, get("V3") := parse_dates(get("V3"))]
